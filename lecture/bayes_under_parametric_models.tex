...
\hfill (The problem continues to the next page.)
\end{frame}


\begin{frame}
\frametitle{Homework: Gaussian likelihood models}
\textbf{Problem 2 (cont'd):}
Next, we study the effect of centering variables. 
...
0 = - \log\!\left( \int q(x) \diff x \right) \pause
	= - \log\!\left( \int \frac{q(x)}{p(x)} p(x) \diff x \right) \pause

...
%\includeonlyframes{working} % Uncomment and use `\begin{frame}[label=working]` for compiling only a subset of slides for speed
...
\newcommand{\map}[1]{\widehat{#1}_{\scriptscriptstyle \mathrm{MAP}}}
...
\newcommand{\bx}{\bm{x}}
\newcommand{\bX}{\bm{X}}
\newcommand{\bXGramMat}{\bX^\transpose \mskip-.7\thinmuskip \bX}
\newcommand{\bmu}{\bm{\mu}}
...
\begin{frame}<handout:0> % Exclude from handout b/c the `only` overlay specificaitons mess up the slide
\frametitle{Example: estimating incidence proportion}
\vspace{-.3\baselineskip}
\begin{minipage}[t][4\baselineskip]{\linewidth}

	\begin{minipage}[t][1.2\baselineskip]{\linewidth}
	\textbf{Question:}\hspace{3.33pt}%
	\only<+>{\fontdimen2\font=.65ex ``Does the new vaccine cause \ldots?''}%
	\only<+->{\fontdimen2\font=.65ex ``Does the new vaccine cause Guillain-Barré syndrome?''}
...
\begin{frame}<beamer:0> % Exclude the `only` overlay specifications for handout, 
\frametitle{Example: estimating incidence proportion}
\vspace{-.3\baselineskip}
\begin{minipage}[t][4\baselineskip]{\linewidth}
	...
	\begin{minipage}[t][1.2\baselineskip]{\linewidth}
	\textbf{Question:}\hspace{3.33pt}%
	{\fontdimen2\font=.65ex ``Does the new vaccine cause Guillain-Barré syndrome?''}
	% Note: Guillain-Barré --- rare but potentially a life-threatening disorder with body's immune system attacking nerves and causing muscle weakness
	\end{minipage}
...
\newcommand{\frametitleText}{Bayesian estimation of incidence proportion}
\begin{frame}
...
\renewcommand{\frametitleText}{Choosing a prior on incidence proportion}
\begin{frame}
\frametitle{\frametitleText}
...
\renewcommand{\frametitleText}{Choosing a informed-but-pragramtic prior on prop}
\begin{frame}
\frametitle{\frametitleText}
\vspace*{-.3\baselineskip}
...
\pause
\begin{minipage}[t][.35\paperheight]{\textwidth}
For example, we can 
\begin{itemize}
\addtolength\itemsep{-.1\baselineskip}
...
\medskip % Not sure why the inter-line spacing gets weird inside a minipage

\pause
In other words, specify a reasonable lower bound $\ell$ and upper bound $u$ on the parameter's range, and solve for $a$ and $b$ such that
\begin{tightEquation*}
\int_0^\ell \density(p \given a, b) \diff x 
	= 0.025
	\ \text{ and } \,
\int_u^1 \density(p \given a, b) \diff x 
	= 0.975.
\end{tightEquation*}
\end{minipage}
...
\renewcommand{\frametitleText}{Choosing a prior on prop: ``objective'' approach}
\begin{frame}
\frametitle{\frametitleText}
...
\onslide<3>{%
At the least, compared to ad hoc ``correction,'' Bayes provides an estimator with explicit and clear interpretation of the prior's role.
}
\end{frame}
...
\frametitle{Homework}
\textbf{Problem 1:} 
The Jeffreys prior is defined as $\density(\btheta) = \det\!\left(\infoMat(\btheta) \right)^{1/2}$, where $\infoMat(\btheta)$ is the Fisher information, and is arguably the prior of choice for objective Bayesian analysis when $\btheta$ is univariate.
Show that $p \sim \betaDist(\frac12, \frac12)$ is the Jeffreys prior for the bernoulli model.
What would be the Jeffreys prior if we use an alternate parametrization $\log \frac{p}{1 - p}$? 
How does that affect posterior inference?
\end{frame}
...
\frametitle{Homework}
\textbf{Problem 2:} 
Consider posterior inference for binomial data $y_1, \ldots, y_{25} \given p \iidSim \mathrm{Bernoulli}(p)$ under the prior $p \sim \betaDist(\frac12, \frac12)$.
Numerically study the frequentist coverage of Bayesian 95\% credible intervals under $p = \frac{k}{100}$ for $k = 1, \ldots, 99$.
You may analytically/numerically calculate any necessary quantities or resort to Monte Carlo simulation.
Compare the credible intervals' performance to the coverage achieved by a frequentist procedure, such as the one based on the asymptotic normality of {\small MLE} or any other techniques that may be better suited to this specific setting.
\end{frame}
...
\renewcommand{\frametitleText}{Univariate case: mean unknown}
\begin{frame}
\frametitle{\frametitleText}
...
\renewcommand{\frametitleText}{Univariate case: unknown variance}
\begin{frame}
\frametitle{\frametitleText}
\pause
...
{\textstyle \argmax_{\phi}} \density\!\left( \phi \given \bm{y}, \mu \right)
	= \left( \frac{\textstyle \sum_i (y_i - \mu)^2}{n - 2} \right)^{-1}
	\! \text{ for }  n \geq 3 
	\, \text{ (and $= 0$ o.w.)}
\end{tightEquation*}
\end{frame}
...
\renewcommand{\frametitleText}{Univariate case: mean and variance unknown}
\begin{frame}
\frametitle{\frametitleText}
...
\likelihood(\bm{y} \given \mu, \phi )
	\propto 
		\gammaDist\!\left( \phi \, \middle| \,  \frac{n}{2} + 1, \, \frac{\sum_i (y_i - \bar{y})^2}{2} \right)  
...
\begin{equation*}
\expectation[ \phi \given \bm{y} ]^{-1}
	= \frac{1}{\nu_0 + n} \left(
		\nu_0 \varphi_0^{-1} 
		+ n \thinnerspace \frac{ \textstyle \sum_i (y_i - \bar{y})^2 }{ n }
		+ \frac{\kappa_0 n}{\kappa_0 + n} (\mu_0 - \bar{y} )^2
	\right).
\end{equation*}

\pause
The $\frac{\kappa_0 n}{\kappa_0 + n} (\mu_0 - \bar{y} )^2$ part shows one way in which $\mu$ and $\phi^{-1}$ interact:
\begin{tightEquation*}
\text{%
	discrepancy between $\mu_0$ and $\bar{y}$ % Note: $\mu_0$ = prior guess and $\bar{y}$ = empirical mean 
	$\, \Rightarrow \,$ inflated estimate of $\phi^{-1}$.
}%
\end{tightEquation*}
...
\begin{frame}
\frametitle{\frametitleText}
The prior has a feature that the apriori (conditional) variance of mean $\variance(\mu \given \phi) = 1 / \kappa_0 \phi$ depends on the noise variance $1 / \phi$.

\pause
\smallskip
The prior thus has the behavior ``larger the $\phi^{-1}$ and noisier the $y_i$'s, less informative the prior on $\mu \given \phi$.''
...
\renewcommand{\frametitleText}{Univariate case: objective Bayeian analysis}
\begin{frame}
\frametitle{\frametitleText}
...
\section{\centerline{Linear regression}}


\begin{frame}
\frametitle{Linear model with Gaussian errors}
...
\likelihood(\bm{y} \given \bX, \bbeta, \phi)
	...
	&\propto \phi^{n / 2} \exp\!\left(
		- \frac{1}{2} \left( 
			\phi \thinnerspace \bbeta^\transpose \bXGramMat \bbeta
...
\normalDist\!\left( 
	...
	\left( \bXGramMat \right)^{-1} \bX^\transpose \bm{y},
	\left( \phi \thinnerspace \bXGramMat \right)^{-1} 
\right).
\end{tightEquation*}
...
\end{aligned}
\end{tightEquation*}
where $\bPhi_{\textrm{post}} = \phi \thinnerspace \bXGramMat + \bPhi_0$ and $\bmu_{\textrm{post}} = \bPhi_{\textrm{post}}^{-1} \left( \phi \thinnerspace \bX^\transpose \bm{y} + \bPhi_0 \bmu_0 \right)$.
\end{frame}
...
\bbeta \given \bm{y}, \bX, \phi
	...
	\right)
\end{tightEquation*}
for $\bmu_{\textrm{post}} = \bPhi_{\textrm{post}}^{-1} \left( \phi \thinnerspace \bX^\transpose \bm{y} + \bPhi_0 \bmu_0 \right)$ and $\bPhi_{\textrm{post}} = \phi \thinnerspace \bXGramMat + \bPhi_0$.

\pause
\smallskip
Note the posterior mean is again a weighted average:
\begin{tightEquation*}
\setlength{\abovedisplayskip}{.3\baselineskip}
\bmu_{\textrm{post}} 
	= \bPhi_{\textrm{post}}^{-1} \left( 
		\phi \thinnerspace \bXGramMat \thinnerspace \mle{\bbeta} + \bPhi_0 \bmu_0 
	\right)
	\text{ for } \thinnerspace
	\mle{\bbeta} = (\bXGramMat)^{-1} \bX^\transpose \bm{y}.
\end{tightEquation*}
\end{frame}
...
Similarly, since the likelihood can be expressed as
\begin{tightEquation*}
\likelihood(\bm{y} \given \bX, \bbeta, \phi)
	\propto \phi^{n / 2} \exp\!\left(
		- \frac{\phi}{2} \left\| \bm{y} - \bX \bbeta \right\|^2 
	\right),
\end{tightEquation*}
we obtain the posterior conditional for $\phi$ as:
\begin{tightEquation*}
...
\pause
\smallskip
And, yet again, the posterior mean is a weighted average:
\begin{tightEquation*}
\setlength{\abovedisplayskip}{.25\baselineskip}
\expectation\bigl[ \phi \given \bm{y}, \bX, \bbeta \bigr]^{-1}
	= \frac{ \nu_0 \varphi_0^{-1} + n \thinnerspace \mle{\sigma}^2 }{ \nu_0 + n }
	\ \text{ where } \, 
	\mle{\sigma}^2 = \frac{ \| \bm{y} - \bX \bbeta \|^2 }{n}.
\end{tightEquation*}
\end{frame}
...
\bbeta \given \bm{y}, \bX, \phi
	\sim \normalDist\!\left(
		 \left(  \bXGramMat \right)^{-1} \! \bX^\transpose \bm{y}, \,
		\left( \phi \bXGramMat \right)^{-1}
	\right), \\
\phi \given \bm{y}, \bX
	\sim \gammaDist\!\left( 
		\frac{n - \nPred}{2}, \thinnerspace \frac{\| \bm{y} - \bX \mle{\bbeta} \|^2}{2} 
	\right). 
	% Note: $\mle{\bbeta} = \left( \bXGramMat \right)^{-1} \! \bX^\transpose \bm{y}$
\end{gathered}
\end{tightEquation*}
...
\item Center $\bm{y}$ and $\bx_j$'s. % all (both cont.\ and binary) $\bx_j$'s.
...
\pause
\smallskip
Gamma prior on $\phi$ is less flexible but, based the interpretation of $\nu_0$ as the prior degrees of freedom,
	% Note: In particular, the right exponential tail necessarily penalizes larger $\phi$ and the left polynomial tail cannot aggressively penalize small $\phi$.
\begin{tightEquation*}
\phi \sim \gammaDist(\nu_0 / 2, \nu_0 / 2)
...
\begin{tightEquation*}
\begin{gathered}
\beta_{\mathrm{cont}, \thinnerspace j} \sim\normalDist\!\left( 0, \, \frac{ \empiricalVar(\by) }{ 4 \empiricalVar(\bx_j) } \right), \ \,
\beta_{\mathrm{bin}, \thinnerspace j} \sim\normalDist\!\left( 0, \, \frac{ \empiricalVar(\by) }{ \empiricalVar(\bx_j) } \right), \\
\beta_0 \sim \normalDist\!\left( 0, \empiricalVar(\by) \right), \ \,
\phi \sim \gammaDist\!\left(
	\frac{\nu_0}{2}, 
	\frac{\nu_0}{2} \thinnerspace \empiricalVar(\by)
\right).
...
\begin{tightEquation*}
\beta_0 \sim \normalDist\!\left( \bar{y}, \thinnerspace \empiricalVar(\by) \right).
\end{tightEquation*}
\end{frame}
...
\frametitle{Informal objective-ish prior in linear regression}
\textbf{Note:}\\
Don't like the use of empirical quantities in the prior? 
Guess what, you often implicitly commit the same crime in frequentist methods.
\end{frame}


\begin{frame}
\frametitle{Homework: Gaussian likelihood models}
%First problem is optional for those who have taken ``Advanced Methods in Biostatistics'' and the second optional for the rest.
\textbf{Note:} Problem 1 and 2(b) are optional.

\textbf{Problem 1:} Verify that the posterior under the model $y_i \given \mu, \phi \sim \normalDist(\mu, \phi^{-1})$ and conjugate prior 
\begin{tightEquation*}
\phi \sim \gammaDist\!\left( \frac{\nu_0}{2}, \thinnerspace \frac{\nu_0 \varphi_0^{-1}}{2} \right), \
\mu \given \phi \sim \normalDist\!\left( \mu_0, \thinnerspace \frac{1}{\kappa_0 \phi} \right)
\end{tightEquation*}
...
\frametitle{Homework: Gaussian likelihood models}
\textbf{Problem 2(a):}
%In fitting a regression model to an outcome $\by$ and predictors $\bx_j$, consider scaling 
...
\frametitle{Homework: Gaussian likelihood models}
\textbf{Problem 2(b):}
We now study the effect of centering variables. 
For this purpose, as you will find out, it is useful to separate out the intercept term from the design matrix and consider the model $\by = \beta_0 \bm{1} + \bX \bbeta + \bm{\epsilon}$ with $\bm{1}$ denoting a vector of $1$'s.
Let $\by_{\textrm{ctr}} = \by - \bar{y} \thinnerspace \bm{1}$ and $\bx_{\mathrm{ctr}, \thinnerspace j} = \bx_j - \bar{x}_j \bm{1}$, where $\bar{y} = n^{-1} \sum_i y_i$ and $\bar{x}_j = n^{-1} \sum_i x_{ij}$.
%In this part of the problem, it is convenient to separate out the intercept $\beta_0$ from the rest of coefficients $\bbeta$.
Consider fitting the regression model with $\by_\mathrm{ctr}$ and $\bX_\mathrm{ctr}$ (i.e.\ $\by_\mathrm{ctr} = \beta_0 \bm{1} + \bX_\mathrm{ctr} \bbeta + \bm{\epsilon}$) under a prior $\beta_0 \sim \normalDist(0, 1)$, $\bbeta \sim \normalDist(\bm{0}, \Id)$, and $\phi \sim \gammaDist(a, b)$.
If we were to fit the regression model to the original variables $\by$ and $\bX$, how must we modify the prior to obtain an equivalent inference?
\end{frame}
...
We've seen examples in which posteriors yield comparable, or even identical, inferences as established frequentist procedures.
...
\pause
\textit{Proof:} 
Jensen's ineq.\ says $f(\expectation[g(X)]) \leq \expectation[f(g(X))]$ for convex $f$, with the equality only if $f(x) = x$ on the support of $\density_X(\cdot)$.
\pause%
Hence,
\begin{tightEquation*}
...
\begin{aligned}
\log\!\left( 
	\frac{\density(\btheta \given \by^n)}{\density(\btheta_\truthSub \given \by^n)}
\right)
	&= \log\!\left( 
		\frac{%
...
\pause%
\smallskip
Now we only need to recognize the latter term as a negative KL:
\smallskip
\begin{tightEquation*}
...
i.e.\ $\density(\thinnerspace \cdot \given \by^n) \to \delta_{\btheta_\truthSub}(\cdot)$.
	% Note: as long as the parameter is identifiable.
\end{frame}

\begin{frame}
\frametitle{Posterior concentration under mis-specified model}
Analogous result holds when a model is \textit{mis-specified} ---
the family $\{ \likelihood(\thinnerspace \cdot \given \btheta) \}_{\btheta}$ doesn't include the true data dist $y \sim \likelihood_\truthSub(\cdot)$; e.g.\
\begin{narrowItemize}[<+->]
...
\end{tightEquation*}
\pause%
Since the posterior concentrates around $\samplesizeDependentMap \approx \theta_\truthSub$, the terms $\big( \theta - \samplesizeDependentMap \big)^3 \frac{\diff^3}{\diff \theta^3} ...$ becomes immaterial relative to $\big( \theta - \samplesizeDependentMap \big)^2 \frac{\diff^2}{\diff \theta^2} ...$. 
\hfill \qedsymbol
\end{frame}
...
\begin{frame}
\frametitle{``Counterexample'' to the asymptotic theory}
In deriving the posterior, note the likelihood can be expressed as
\begin{tightEquation*}
\setlength{\abovedisplayskip}{.4\baselineskip}
\begin{aligned}
&L( y_{i1}, y_{i2} \given \mu_i, \phi ) \\
&\hspace*{1em}\propto
	\phi^{\frac12}
	\exp\!\left(
		- \frac{\phi}{2} \Bigl( 
...
&\hspace*{1em}\propto
	\phi^{\frac12}
	\exp\!\left(
		- \frac{\phi}{2} \!\left( \frac{y_{i1} - y_{i2}}{2} \right)^2 
	\right)
	\exp\!\left(
		- \frac{\phi}{2} \!\left( \mu_i - \frac{y_{i1} + y_{i2}}{2} \right)^2 
	\right)
\end{aligned}
\end{tightEquation*}
since
$\displaystyle 
	\sum_{j = 1}^2 (y_{ij} - \mu_i)^2 
	= \sum_{j = 1}^2 (y_{ij} - \bar{\mu}_{i})^2 + 2 (\bar{\mu}_{i} - \mu_i)^2$ \\[-6pt]
\hfill for \, $\displaystyle \bar{\mu}_{i} = \frac{y_{i1} + y_{i2}}{2}$.
\end{frame}
...
\mu_i \given \bm{y}, \phi 
	\sim \normalDist\!\left( 
		\frac{y_{i1} + y_{i2}}{2}, \thinnerspace
		\frac12 \phi^{-1}
	\right), \\
\phi \given \bm{y}
	\sim \gammaDist\!\left( 
		\frac{n}{2}, \thinnerspace 
		\frac{\sum_i (y_{i1} - y_{i2})^2}{4} 
	\right).
\end{gathered}
\end{tightEquation*}
...
\begin{tightEquation*}
\expectation[\mu_i \given \bm{y}]
%	= \expectation[ \expectation[\mu_i \given \bm{y}, \phi] ]
	= \frac{y_{i1} + y_{i2}}{2}, \ \,
\expectation[\phi \given \bm{y}]^{-1}
	= \frac{\sum_i (y_{i1} - y_{i2})^2}{2 n}.
\end{tightEquation*}
...
\pause
\smallskip
TODO: derive MLE and, from it, MAP from it.
%But plugging in the mode $\bar{\mu}_i$ and maximizing 
%\begin{tightEquation*}
%\setlength{\abovedisplayskip}{.35\baselineskip}
%\begin{aligned}
%&\prod_i L( y_{i1}, y_{i2} \given \bar{\mu}_i, \phi ) \\
%&\hspace*{1em}\propto
%	\phi^{n / 2}
%		\exp\!\left(
%			- \frac{\phi}{2}
%				\sum_i \!\left( \frac{y_{i1} - y_{i2}}{2} \right)^2 
%		\right)
%\end{aligned}
%\end{tightEquation*}
...
%But the posterior mode (which is similar to the {\small MLE}) exhibits a fundamentally different behavior, being off by a factor of 2:
\begin{tightEquation*}
\map{\phi}^{-1} = \frac{\sum_i (y_{i1} - y_{i2})^2}{4 (n - 1)}.
\end{tightEquation*}
% Note: Worth working this out in the class for students to really believe it.
...
\end{tightEquation*}
which \textit{integrates out} $\bmu$, uncertainty in which remains as $n \to \infty$.

\pause%
\smallskip
%Incidentally, the posterior marginal can be equivalently derived as $\propto \likelihood(\by \given \phi) \density(\phi)$ in terms of the marginal likelihood
%\begin{tightEquation*}
%\likelihood(\by \given \phi)
%%	= \int \likelihood(\by, \bmu \given \phi) \diff \bmu
%	= \int \likelihood(\by \given \bmu, \phi) \density(\bmu \given \phi) \diff \bmu.
%\end{tightEquation*}
Its effect is also seen in the fact $\thinnerspace \variance(\phi \given \by) \sim 4 n / \phi_\truthSub^2$ while 
\begin{tightEquation*}
- \frac{\diff^2}{\diff (\bmu, \phi)^2} \log \likelihood(y \given \bmu, \phi)
	= \begin{bmatrix}
	\phi \Id_{n \times n} & \bm{0} \\
	0 & n / \phi^2
	\end{bmatrix}.
\end{tightEquation*}
\end{frame}
...
\begin{frame}
\frametitle{Summary of ``Bayes under parametric models''}
We have studied behaviors of posteriors under relatively simple models to build intuitions. 

\pause%
Under parametric models, Bayesian inference has much similarity to the classical one based on {\small MLE}.
\pause%
\begin{narrowItemize}[<+->]
\item Conjugate analysis demonstrates sound, intuitive behaviors of posteriors and helps quantify relative weights on priors. 
	% Note: Having been trained in the classical statistics, we cannot help notice the familiar quantities popping up in the Bayesian analysis of the models. But the only thing we did really was just to adhere to what the probability theory tells us and multiply the likelihood and prior. We woud have arrived at a right statistical inference even if we knew nothing about the classical statistical estimation methods.
	% Note: Many Bayes newbies are, understandably, uncomfortable at picking a prior. But the prior really doesn't have much impact on the posterior when the likelihood is informative and, when the likelihood is not, then we cannot really proceed without imposing some additional structures.
\item Bayesian cred intervals often have remarkably good freq performance (despite the Bayes-freq philosophical differences).
\item The two``CI''s coincide asymptotically; 
	Bayes's may well have better freq performance than alternatives in finite samples.
%\item In fact, Bayesian thinking can be useful in devising a freq procedure, whose performance can then be evaluated formally.
\end{narrowItemize}
\end{frame}
...
\end{indented}
%\begin{narrowItemize}
%\item \bda{} Chapter~2/3 for uni-/multi-variate models
%\end{narrowItemize}
...
\item \cite{bayarri2004bayes_freq_interplay}.% 
	``\usebibentry{bayarri2004bayes_freq_interplay}{title}.''%
	\textit{\usebibentry{bayarri2004bayes_freq_interplay}{journal}}.
\end{narrowItemize}
\end{frame}

...
\end{aligned}
\end{tightEquation*}
b/c $\sum_i (y_i - \mu)^2 = \sum_i (y_i - \bar{y})^2 + \sum_i (\bar{y} - \mu)^2$ for $\bar{y} =  \sum_i y_i \divby n$.
...
\begin{frame}
\frametitle{Univariate case with unknown variance}
We can now see the following prior as conjugate:
\begin{tightEquation*}
\phi \sim \gammaDist\!\left( \frac{\nu_0}{2}, \thinnerspace \frac{\nu_0 \varphi_0^{-1}}{2} \right), \
...
&\phi \given \bm{y} 
	\sim \gammaDist\!\left(
		\frac{\nu_0 + n}{2},
		\frac{\nu_0 \varphi_0^{-1} + \textstyle \sum_i (y_i - \bar{y})^2}{2} 
	\right), \\
&\mu \given \bm{y}, \phi 
...
\right]
	= \left( 
		\bar{y}, \, \frac{n}{\textstyle \sum_i (y_i - \bar{y})^2}
	\right).
\end{tightEquation*}
\end{frame}

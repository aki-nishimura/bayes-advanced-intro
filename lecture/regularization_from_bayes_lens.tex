...
\newcommand{\bookreference}[1]{%
	\cite{#1}. \textit{\usebibentry{#1}{title}}.
}
...
\newcommand{\js}[1]{\widehat{#1}_{\scriptscriptstyle \mathrm{JS}}}
...
\begin{tightEquation*}
\js{\bmu}(\by) = \left( 1 - \frac{(n - 2) \sigma^2}{\| \by \|^2} \right) \by
\end{tightEquation*}
...
\pause%
\smallskip
Technically, ``shrinkage'' in $\js{\bmu}$ is only true with high probability: 
...
\pause%
\smallskip
Better yet, the following modification improves on $\js{\bmu}$:
\begin{tightEquation*}
\widehat{\bmu}_{\scriptscriptstyle \mathrm{B}}(\by) = \left[ 1 - \frac{(n - 2) \sigma^2}{\| \by \|^2} \right]^+ \! \by.
...
\pause%
We now calculate \textsc{SURE} for $\, \js{\bmu}(\by) = \left( 1 - \frac{(n - 2) \sigma^2}{\| \by \|^2} \right) \! \by$.
\pause% 
Note
\begin{equation*} \defineTightSpacing%
\| \by - \js{\bmu} \|^2 
...
and
\begin{equation*} \defineTightSpacing%
\frac{\partial \widehat{\mu}_{{\scriptscriptstyle \mathrm{JS}}, i}}{\partial y_i}
...
\begin{equation*} \defineTightSpacing%
{\textstyle \sum_i} \frac{\partial \widehat{\mu}_{{\scriptscriptstyle \mathrm{JS}}, i}}{\partial y_i}
...
\begin{frame}
\frametitle{M{\normalsize SE} of Gaussian mean estimator}
So \textsc{SURE} for $\js{\bmu}$ is given by
\begin{equation*} \defineTightSpacing%
\begin{aligned}
\js{\operatorname{MSE}}
	&= - n \sigma^2 
...
\pause%
Using this, we can study the asymptotic behavior of $\js{\bmu}$'s \textsc{MSE}:
\begin{equation*} \defineTightSpacing%
\begin{aligned}
\expectation \| \js{\bmu} - \bmu \|^2 
	&= n \sigma^2  - (n - 2)^2 \sigma^4 \thinnerspace \expectation\!\left[ \frac{\!\!1}{\| \by \|^2} \right] \\
...
\end{equation*}
which also dominates $\mle{\bmu}$ in {\small MSE} if not quite as much as $\js{\bmu}$.
\end{frame}
...
\pause
And $\likelihood(\by \given \widehat{\phi}_{\mathrm{grp}})$ is the most compatible with the observed $\by$.
...
\begin{equation*} \defineTightSpacing%
\loss_\mathrm{ba}(y_{\trainingSize + 1}, \modelSymbol)
...
As discussed earlier, for a chosen loss $\loss(y, \widehat{y})$, Bayesian paradigm considers its average over the posterior:
\begin{equation*} \defineTightSpacing%
\loss_\mathrm{bayes}(y_{\trainingSize + 1}, \modelSymbol)
...
%
utations, $\trainingSize!$ of them correspond to a fixed set of $\{ \permutation(1), \ldots, \permutation(\trainingSize) \}$.
...
\begin{frame}
\frametitle{Summary of \\ \hspace*{1.5em}``Regularization/shrinkage from Bayes lens''}
Regularization/shrinkage are essential in modern, high-dimensional applications for which variance reduction outweighs modest bias.

\pause
\smallskip
For a probabilistic/likelihood-based model, regularization admits Bayesian interpretation as coming from a prior on unknown $\btheta$:
\vspace*{-.25\baselineskip}
\pause
\begin{itemize}
\item Minimizing $\thinnerspace \loss(\by \given \btheta) + \operatorname{pen}(\btheta \given \lambda)$ is equiv to maximizing $\thinnerspace \exp(- \loss(\by \given \btheta)) \exp(- \operatorname{pen}(\btheta \given \lambda))$ --- the latter has a probabilistic interpretation if both terms are integrable.
\pause
\item But note also that \textsc{map} is not a Bayes estimator.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Summary of \\ \hspace*{1.5em}``Regularization/shrinkage from Bayes lens''}
Model/hyper-parameter selection is typically done via \textsc{cv} under freq paradigm and via marginal likelihood under Bayes. 

\pause
\smallskip
But the two frameworks are connected, with log marginal likelihood being equivalent to cross-validated log-predictive score. 

\pause
\smallskip
Bayes lacks frequentists' flexibility to pick an arbitrary loss function, but has the advantage of \textsc{cv} \textit{built into} its inference. 
\pause
{\reduceLeftmargin
\begin{tightItemize}
\item For flexible models (e.g.\ regression trees), estimating prediction error can be difficult and Bayes may have an advantage.
\end{tightItemize}
}
\end{frame}
...
Stein's estimator and its Bayesian interpretation:
\begin{tightItemize}
\item \reference{efron1973stein_est}
\end{tightItemize}

\pause
Bayesian decision theory:
\begin{tightItemize}
\item Chapter~1.1--1.5 of \bookreference{berger1985decision_theory}
\item Chapter~5.7 of \bookreference{murphy2012probabilistic_ml}
\end{tightItemize}
\end{frame}
...
\begin{frame}
\frametitle{References}
Regularized regression and its Bayesian interpretation:
\begin{tightItemize}
\item Chapter~3 of \bookreference{bishop2006pattern_recognition}
\item Chapter~7.5 \& 7.6 of \bookreference{murphy2012probabilistic_ml}
\item \reference{park2008bayesian_lasso}
\end{tightItemize}

\pause
Connection between marginal likelihood and \textsc{cv}:
\begin{tightItemize}
\item \reference{fong2020marginal_likelihood}
\end{tightItemize}
\end{frame}

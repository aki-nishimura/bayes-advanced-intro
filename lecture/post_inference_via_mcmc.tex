...
%% !TeX program = xelatex
%\includeonlyframes{inprogress} % Use \begin{frame}[label=inprogress] to edit designated frames
...
Instead of i.i.d.\ draws, \mcmc{} generates a Markovian sequence $\randVar^{(1)}, \randVar^{(2)}, \ldots$ whose stationary distribution coincides with $\density(\cdot)$; 
\pause%
i.e.%
\vspace*{-.2\baselineskip}%
\begin{itemize}
...
\vspace*{-.2\baselineskip}
\textbf{Example {\normalfont (of Markov chain, not of \mcmc{})}:}
\begin{tightEquation*}
\setlength{\belowdisplayskip}{-.15\baselineskip}
\randVar^{(\mcIndex + 1)} 
	= \rho \thinnerspace \randVar^{(\mcIndex)} + \sqrt{1 - \rho^2} \thinspace \epsilon_{k + 1}
\end{tightEquation*}
or equivalently
\begin{tightEquation*}
\randVar^{(\mcIndex + 1)} \given \randVar^{(\mcIndex)}, \ldots, \randVar^{(1)} 
%	\eqDistribution \randVar^{(\mcIndex + 1)} \given \randVar^{(\mcIndex)}
	\sim \normalDist\!\left( \rho \thinnerspace \randVar^{(\mcIndex)}, 1 - \rho^2 \right),
\end{tightEquation*}
which admits $\density(\cdot) \sim \normalDist(0, 1)$ as the stationary distribution.

\pause
On the other hand, something like below won't be Markovian:
\begin{tightEquation*}
\setlength{\belowdisplayskip}{-.15\baselineskip}
\randVar^{(\mcIndex + 1)} 
	= \rho_{-1} \thinnerspace \randVar^{(\mcIndex)} + \ldots + \rho_{-k} \thinnerspace \randVar^{(1)} + \rho_0 \thinnerspace \epsilon_{k + 1}.
\end{tightEquation*}
\end{frame}
...
\end{itemize}
\vspace*{-.3\baselineskip}%
the draws generated by the \mcmc{} algorithm can be used  in the same manner as i.i.d.\ draws to calculate quantities of interest.
...
\section{%
	\centerline{Essential \titleSmallCap{MCMC} algorithms:}
	\centerline{Gibbs and Metropolis}%
}
...
% TODO: make the proof into a homework problem.
\pause
\smallskip
...
\begin{tightItemize}
\item[(a)] Set up the problem with $\density_j( \placeholder ) = \density( \placeholder \given \bm{\randVar}^\nextMarker_{-j} )$ as the target, the $\bm{\randVar}_j \defeq \bm{\randVar}^{(k - 1)}_j$ as the current state;
	\pause%
...
\begin{frame}
\frametitle{\frametitleText}
Recall that, with i.i.d.\ Monte Carlo,
%$\randVar^{(1)}, \randVar^{(2)}, \ldots \sim \density(\cdot)$ 
the error decays as $\mcSampleSize^{-1/2}$ b/c 
$$\mathrm{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\left( \bm{\randVar}^{(\mcIndex)} \right) \right)
	= \frac{1}{\mcSampleSize} 
	\mathrm{var} \! \left( g \!\left( \bm{\randVar} \right) \right).$$
\end{frame}
...
$\sum_{\mcIndex \neq \altMcIndex} = 2 \sum_{\lag = 1}^\mcSampleSize \sum_{\mcIndex, \altMcIndex \spacedColon \thinnerspace \mcIndex - \altMcIndex \spacedEq \lag} \thinnerspace$,
\vspace*{-.3\baselineskip}
\begin{align*}
&\mathrm{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\bigl( \brandVar^{(\mcIndex)} \bigr) \right) \\
	&\hspace*{1em}= \frac{1}{\mcSampleSize} \mathrm{var} \!\left( g(\brandVar) \right)
	+ \frac{2}{\mcSampleSize^2} \sum_{\lag = 1}^\mcSampleSize (\mcSampleSize - \lag) \operatorname{cov} \!\left( g\bigl( \brandVar^{(\lag + 1)} \bigr), g\bigl( \brandVar^{(1)} \bigr)  \right).
\end{align*}
\end{frame}
...
\begin{frame}
\frametitle{\frametitleText}
%As $\mcSampleSize \to \infty$, which also guarantees stationarity, 
So we see that
\vspace*{-.15\baselineskip}
{\setlength{\belowdisplayskip}{.4\baselineskip}
\begin{align*}
&\mcSampleSize \operatorname{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\bigl( \brandVar^{(\mcIndex)} \bigr) \right) \\
	...
	+ 2 \sum_{\lag = 1}^\mcSampleSize \left( 1 - \frac{\lag}{\mcSampleSize} \right) \operatorname{cov} \!\left( g\bigl( \brandVar^{(\lag + 1)} \bigr), g\bigl( \brandVar^{(1)} \bigr)  \right) \\
	&\hspace*{1em}\to
		\mathrm{var} \!\left( g(\brandVar) \right)
		+ 2 \sum_{\lag = 1}^\infty \operatorname{cov} \!\left( g\bigl( \brandVar^{(\lag + 1)} \bigr), g\bigl( \brandVar^{(1)} \bigr)  \right)
		\ \text{ as } \mcSampleSize \to \infty.
\end{align*}
}%

%Note the limit $\mcSampleSize \to \infty$ also guarantees stationarity; 
%i.e.\ $\brandVar^{(\mcIndex)}$ is effectively at stationarity for large $\mcIndex$, regardless of initialization.%
Note that the above formula holds regardless of initialization b/c $\mcSampleSize \to \infty$ also guarantees eventual stationarity.
...
We have essentially shown that, for large enough $\mcSampleSize$, 
\begin{tightEquation*}
\begin{aligned}
&\operatorname{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\bigl( \brandVar^{(\mcIndex)} \bigr) \right) \\
	&\hspace*{1.5em}\approx 
	\frac{1}{\mcSampleSize} \left[
		\mathrm{var} \!\left( g(\brandVar) \right)
			+ 2 \sum_{\lag = 1}^\infty \operatorname{cov} \!\left( g\bigl( \brandVar^{(\lag + 1)} \bigr), g\bigl( \brandVar^{(1)} \bigr) \right)
	\right].
\end{aligned}
...
\pause
In other words, error of an \mcmc{} estimator decays as $\mcSampleSize^{-1/2}$, but is inflated relative to the i.i.d.\ estimator by the factor
\vspace*{-.5\baselineskip}
\begin{equation*}
\left[
1 + 2 \sum_{\lag = 1}^\infty \operatorname{cor} \!\left( g\bigl( \brandVar^{(\lag + 1)} \bigr), g\bigl( \brandVar^{(1)} \bigr)  \right)
\right]^{1/2}.
\end{equation*}
\end{frame}
...
\begin{equation*}
\operatorname{ESS}_g (\mcSampleSize) =
\frac{\mcSampleSize}{
	1 + 2 \sum_{\lag = 1}^\infty \operatorname{cor} \!\left( g\bigl( \brandVar^{(\lag + 1)} \bigr), g\bigl( \brandVar^{(1)} \bigr)  \right)
}
\end{equation*}
b/c the estimator's variance equals that of $\operatorname{ESS}_g (\mcSampleSize)$ i.i.d.\ samples.
...
\pause
\medskip
Let's first try answering this question with tuning set aside.\\
...
\begin{frame}
\frametitle{\frametitleText}
In figuring out when $\log \density\bigl(\brandVar^{(\mcIndex)} \bigr)$ looks stable enough, helpful to think about what its stationary distribution (approximately) is.

\pause
\medskip
If the target is $\normalDist \bigl( \bm{0}, \bm{\Sigma} \bigr)$, then $\operatorname{var}\bigl(\thinnerspace \log \density( \brandVar ) \bigr) = \nParam \divby 2 \thinnerspace$ because
\begin{tightEquation*}
- \log \density( \brandVar ) + \operatorname{const}
	= \textstyle \frac12 \brandVar^\transpose \bm{\Sigma}^{-1} \brandVar
	\sim \frac12 \chi^2_{\nParam};
\end{tightEquation*}
so we expect $\log \density\bigl(\brandVar^{(\mcIndex)} \bigr)$ to fluctuate roughly by $2 \times 1.96 \sqrt{\nParam \divby 2} $.

\pause
\medskip
Above is a good rule of thumb more generally, esp.\ as parametric model posteriors are approx Gaussian under large-sample limits.
\end{frame}
...
\frametitle{\frametitleText}
\textbf{Remark:} 
Note that $\thinnerspace \expectation\left[ \log \density( \brandVar ) \right] =  \log \density( \bparam_\mathrm{mle} ) - \nParam \divby 2$;
\smallCap{mle} \textit{isn't the} representative parameter value from a Bayesian perspective. 

\pause
\smallskip
This feature of Bayesian inference also relates to the phenomenon that ``flatter'' optima often generalize better than ``sharper'' ones.\\
{\hfill \citep{hochreiter1997flat_minima, keskar2017large_batch_training}}
\end{frame}


\begin{frame}
\frametitle{\frametitleText}
When tuning is involved, that part of the chain too needs discarding (unless diminishing adaptation used).

In fact, the tuning generally needs to continue beyond the optimization phase to ensure proper adaptation to the target.

\vspace*{9pt}
This means, by the time you are done tuning, you should also be done burning in and shouldn't have to discard additional iterations.
\end{frame}
...
No clear-cut guidance, unfortunately; heuristics or trial-and-errors often required.

\pause
Stan's default is to use half the total iterations as the \textit{warm-up}, the combined burn-in and tuning phase.

\vspace*{9pt}
Trace of $\thinnerspace \log \density\bigl(\brandVar^{(\mcIndex)} \bigr)$ remains useful in diagnosing potential issues in the post warm-up samples. 
\end{frame}
...
\begin{frame}
\frametitle{\frametitleText}
Monitoring of log density/other key param values isn't fail-proof.
...
\section{%
	\centerline{Hamiltonian Monte Carlo:} 
	\centerline{modern general-purpose sampler}%
}

\renewcommand{\frametitleText}{Why Hamiltonian Monte Carlo?}
\begin{frame}
\frametitle{\frametitleText}
\tightSpacing{%
Gibbs sampling in general works well for conjugate models and has served as the engine for early software packages like \smallCap{bugs} and \smallCap{jags}.
}%

(\smallCap{bugs} = Bayesian inference Using Gibbs Sampling)\\
(\smallCap{jags} = Just Another Gibbs Sampler)

\pause
\vspace*{9pt}
%Non-conjugate conditionals can be dealt w/ Metropolis-within-Gibbs steps, but as we've seen, random-walk Metropolis can be inefficient.
Metropolis is more widely applicable and can help Gibbs deal w/ non-conjugate conditionals, but as we've seen, can be inefficient.
\end{frame}
...
\frametitle{\frametitleText}
\textbf{Question:} 
Can we design a better proposal mechanism for Metropolis(-Hastings) than the random-walk one?

\pause
\medskip
In designing a proposal, the two (often competing) desiderata are:%
\begin{tightItemize}
\item sufficiently large move to explore the space;
\item sufficiently high acceptance rate to avoid rejections.
\end{tightItemize}

\pause
\medskip
\tightSpacing{%
Turns out, Hamiltonian dynamics can be used generate proposals that are far away from the current state yet acceptable w/ high prob.
}%
\end{frame}


\renewcommand{\frametitleText}{Metropolis meets Hamilton: \titleSmallCap{hmc} rises}
\begin{frame}
\frametitle{\frametitleText}
Originated in comp physics literature as \textit{hybrid} Monte Carlo for computer simulation of lattice models in quantum field theory.\\
{\hfill \citep{duane1987hybrid_mc}}
% Note: HMC may seem to come from nowhere, so a bit of history to help.

\pause
Two dominant simulation approaches then were:
\begin{tightItemize}
\item Monte Carlo simulation (i.e.\ Metropolis) w/ ``local'' moves, changing the state of one particle at a time.
\item molecular dynamics simulation w/ ``global'' moves, numerically solving an \textit{equation of motion} (i.e.\ Hamiltonian dynamics).
\end{tightItemize}

\pause
\medskip
Hybrid Monte Carlo combined the efficiency of molecular dynamics w/ Metropolis algorithm to exactly sample from the target.
\end{frame}
...
\parbox[c][2.4\baselineskip]{\textwidth}{%
	\only<1>{%
%		Originating in comp physics \citep{duane1987hybrid_mc}, 
		H\smallCap{mc} was popularized in stats/\smallCap{ml} via \cite{neal1996bayes_neural_net,neal2011hmc} and others:
	}%
	\only<2>{%
...
\renewcommand{\frametitleText}{Hamiltonian dynamics as proposal mechanism}
\begin{frame}
\frametitle{\frametitleText}
...
\smallskip
\tightSpacing{%
You can also think of \hmc{} as defining a transition kernel in $\bparam$ only
as the refreshment step makes the previous momentum value irrelevant.
}%
...
\renewcommand{\frametitleText}{Intuition and theory behind \titleSmallCap{hmc} proposals}
\begin{frame}
\frametitle{\frametitleText}
...
\begin{frame}
\frametitle{\frametitleText}
Symmetry of \hmc{} proposals stems from Hamiltonian dynamics' \textit{volume-preserving} property and \textit{time-reversibility}:
% Note: Illustrative examples could be the China-Denmark pair (similar population density) and the Tokyo-Montana pair (similar land mass)
\vspace*{-\baselineskip}
\begin{figure}
\centering
...
%}
\newcommand{\numIntegStep}{L}
\renewcommand{\frametitleText}{Practical details: numerically solving dynamics}
\begin{frame}
\frametitle{\frametitleText}
...
\pause%
\smallskip
Taking $\numIntegStep = \lfloor \integTime / \stepsize \rfloor$ leapfrog steps of size $\stepsize$ approximates the exact solution $(\bposition_0, \bmomentum_0) \to (\bposition_\integTimeScript, \bmomentum_\integTimeScript)$ up to an $O(\stepsize^2)$ error.
\end{frame}


\renewcommand{\frametitleText}{Practical details: tuning of \titleSmallCap{hmc}}
\begin{frame}
\frametitle{\frametitleText}
...
\renewcommand{\frametitleText}{Practical details: tuning of \titleSmallCap{hmc} stepsize}
\begin{frame}
\frametitle{\frametitleText}
...
(Think: with smaller $\stepsize$, you are generating qualitatively similar proposals, but just are spending more computational effort.)
...
\renewcommand{\frametitleText}{Practical details: tuning of \titleSmallCap{hmc} mass matrix}
\begin{frame}
\frametitle{\frametitleText}
\tightSpacing{%
$\bmass$ can be tuned to help \hmc{} handle variations in parameters' scales.
}%

\pause%
\medskip
\Hmc{} in general samples more efficient from a target with less variations in the ``widths'' along different directions.

For example, between $(\randVar_1, \randVar_2) \sim \normalDist(0, 10^2) \times \normalDist(0, 10^2)$ and $(\randVar_1, \randVar_2) \sim \normalDist(0, 1) \times \normalDist(0, 100^2)$, the former is easier for \hmc{}.

\pause%
\vspace*{9pt}
While Stan automatically tunes $\bmass$, you can help it by manually pre-processing (re-scaling/-parametrizing) the model parameters.

\tightSpacing{%
For example in a linear regression model, standardizing the outcome and predictors helps place the coefficients on comparable scales.
}%
\end{frame}


\renewcommand{\frametitleText}{Practical details: tuning stepsize \& mass matrix}
\begin{frame}
\frametitle{\frametitleText}
\medskip
\includegraphics[width=\linewidth]{Figure/stan_warmup_epochs}
%\caption*{\textcolor{themecolor}{Figure:}}

Figure above illustrates Stan's strategy for tuning $\stepsize$ and $\bmass$: % Note: checked last time on Dec 2025
\pause%
\begin{tightItemize}
\item Stage \textrm{I} and \textrm{III} only tune $\stepsize$, which is relatively quick;
\item \tightSpacing{%
	Stage \textrm{II} tunes $\bmass$, along with $\stepsize$, over multiple memoryless windows;
	$\bmass$ from the last window is used in the sampling phase.
	}%
\end{tightItemize}
...
\renewcommand{\frametitleText}{Advantage of \titleSmallCap{hmc}: a bit of theoretical analysis}
\begin{frame}
\frametitle{\frametitleText}
``Ok, I see it got nice properties, but does it \textit{actually} work?'' 
--- this is a fair, worthy question for you to ask.

\pause%
\medskip
We can quantify relative performance btwn \hmc{} and \smallCap{rwm} on $\density(\bparam) = \prod_{j}^{\nParam} \density(\param_j)$ as $\nParam \to \infty$ (though this admittedly isn't realistic).\\
{\hfill \citep{creutz1988global_monte_carlo, beskos2013optimal_hmc}}

\pause%
\medskip
We'll find that, while each \hmc{} iteration costs more than \smallCap{rwm} one, the gain in mixing is well worth the extra per-iteration cost.
\end{frame}
...
\begin{tightItemize}
\item one log-density evaluation ($\propto \nParam$ for $\density(\bparam) = \prod_{j}^{\nParam} \density(\param_j)$) for \smallCap{rwm};
\item \tightSpacing{%
	$\numIntegStep = \lfloor \integTime / \stepsize \rfloor$ log-density + grad evaluations for \smallCap{hmc} (for $\integTime$ fixed).
	}%
\end{tightItemize}

\pause%
\vspace*{1.5pt}
Maintaining a constant acceptance rate (e.g.\ 0.23 or 0.65) requires
\vspace*{-.1\baselineskip}%
\begin{tightItemize}
\item $\sigma_{\mathrm{prop}}^2 \propto \nParam^{-1}$ for \smallCap{rwm}; 
\item $\stepsize \propto \nParam^{-1/4}$ for \smallCap{hmc}.
\end{tightItemize}

\pause%
\vspace*{1.5pt}
Traveling a unit distance requires
\vspace*{-.1\baselineskip}%
\begin{tightItemize}
\item $\propto \nParam$ iterations, due to its random-walk behavior, for \smallCap{rwm}; 
\item $\propto \nParam^{1/4}$ numerical integration steps for \smallCap{hmc}.
\end{tightItemize}

\pause%
\vspace*{2pt}
Grand total cost for one \smallCap{ess}: $O(\nParam^2)$ for \smallCap{rwm}; $O(\nParam^{5/4})$ for \hmc{}.
\end{frame}


\begin{frame}
\frametitle{\frametitleText}
Above result pertains to performance in high dimensions, but \\
\hmc{} also handles correlation in parameters better than \smallCap{rwm}.

(But the theory here remains less complete.)
\end{frame}
...
\begin{frame}
\frametitle{Summary of ``Posterior inference via \titleSmallCap{mcmc}''}

\Mcmc{} remains among the most reliable and versatile tools, with variational inference a popular alternative for point estimation. 

Others available for specialized purposes or under active research.

\pause%
\vspace{10pt}
Metropolis and Gibbs: the most widely used paradigms by existing packages, w/ \hmc{} being the general-purpose sampler of choice.
...
Mixing and cost-per-iteration determine efficiency of \mcmc{}.

E\smallCap{ss} can be used to characterize mixing (and Monte Carlo errors).

\pause%
\vspace{12pt}
\Mcmc{} requires initial burn-in and, often, tuning iterations. 

\vspace{1.5pt}
\Hmc{} is powerful yet also delicate;
Stan has automated tuning and hid details to the extent possible, but some user discretion and occasional manual interventions (e.g.\ reparametrization) warranted.
\end{frame}

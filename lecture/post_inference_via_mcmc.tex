...
- \log \density( \brandVar ) + \operatorname{const}
	...
	\sim \frac12 \chi^2_{\nParam};
\end{tightEquation*}
so we expect $\log \density\bigl(\brandVar^{(\mcIndex)} \bigr)$ to fluctuate roughly by $2 \times 1.96 \sqrt{p \divby 2} $.
...
\begin{frame}
\frametitle{\frametitleText}
\tightSpacing{Story is a bit more complex when tuning is involved b/c that part of the chain too needs discarding (unless diminishing adaptation used).}
...
\vspace*{2pt}
Grand total cost for one \smallCap{ess}: $O(d^2)$ for \smallCap{rwm}; $O(d^{5/4})$ for \hmc{}.
\end{frame}

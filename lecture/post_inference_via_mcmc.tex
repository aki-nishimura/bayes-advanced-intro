...
For example, $- \log U \sim \mathrm{Exp}(1)$.
\smallskip
...
When $\mass = M \bm{I}$, the equation describes a motion of a particle with mass $M$ under the potential energy field $U(\bparam)$:
...
\begin{figure}
\centering
\includegraphics[width=.7\linewidth, trim={.15\linewidth} {.15\linewidth} {.15\linewidth} {.15\linewidth}, clip]{Figure/harmonic_hmc_trajectory}
\end{figure}
...
\begin{tightEquation*}
\begin{aligned}
\potential(\bparam_0) + \frac12 \bmomentum_0^\transpose \thinnerspace \mass^{-1} \bmomentum_0
	&= \potential(\bparam_\integTimeScript) + \frac12 \bmomentum_\integTimeScript^\transpose \thinnerspace \mass^{-1} \bmomentum_\integTimeScript \\
...
\renewcommand{\frametitleText}{Efficiency of Markov chain}
\begin{frame}
\frametitle{\frametitleText}
...
\vspace*{-.15\baselineskip}
\begin{align*}
&\mathrm{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\bigl( \randVar^{(\mcIndex)} \bigr) \right) \\
	&\hspace*{1em}= \frac{1}{\mcSampleSize^2} \sum_{\mcIndex = 1}^\mcSampleSize \mathrm{var} \!\left( g\bigl( \randVar^{(\mcIndex)} \bigr) \right)
	+ \frac{1}{\mcSampleSize^2} \sum_{\mcIndex \neq \altMcIndex} \mathrm{cov} \!\left( g\bigl( \randVar^{(\mcIndex)} \bigr), g\bigl( \randVar^{(\altMcIndex)} \bigr)  \right)
\end{align*}
%
At stationarity $\randVar^{(1)} \sim \density(\cdot)$, noting
...
\begin{align*}
&\mathrm{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\bigl( \randVar^{(\mcIndex)} \bigr) \right) \\
	&\hspace*{1em}= \frac{1}{\mcSampleSize} \mathrm{var} \!\left( g(\randVar) \right)
	+ \frac{2}{\mcSampleSize^2} \sum_{d = 1}^\mcSampleSize (m - d) \operatorname{cov} \!\left( g\bigl( \randVar^{(d + 1)} \bigr), g\bigl( \randVar^{(1)} \bigr)  \right)
\end{align*}
...
\vspace*{-.15\baselineskip}
\begin{align*}
&\mcSampleSize \operatorname{var} \!\left( \frac{1}{\mcSampleSize} \sum_{\mcIndex = 1}^\mcSampleSize g\bigl( \randVar^{(\mcIndex)} \bigr) \right) \\
	&\hspace*{1em}= 
	\mathrm{var} \!\left( g(\randVar) \right)
	+ 2 \sum_{d = 1}^\mcSampleSize \left( 1 - \frac{d}{m} \right) \operatorname{cov} \!\left( g\bigl( \randVar^{(d + 1)} \bigr), g\bigl( \randVar^{(1)} \bigr)  \right) \\
	&\hspace*{1em}\to
		\mathrm{var} \!\left( g(\randVar) \right)
		+ 2 \sum_{d = 1}^\infty \operatorname{cov} \!\left( g\bigl( \randVar^{(d + 1)} \bigr), g\bigl( \randVar^{(1)} \bigr)  \right). 
\end{align*}
%
...
\begin{equation*}
\left[
1 + 2 \sum_{d = 1}^\infty \operatorname{cor} \!\left( g\bigl( \randVar^{(d + 1)} \bigr), g\bigl( \randVar^{(1)} \bigr)  \right)
\right]^{1/2}.
\end{equation*}

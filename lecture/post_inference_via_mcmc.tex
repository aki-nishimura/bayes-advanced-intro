...
\begin{frame}
\frametitle{\frametitleText}
``Ok, I see it got nice properties, but does it \textit{actually} work?''\! --- fair Q.
...
\item \tightSpacing{%
	$\numIntegStep = \lfloor \integTime / \stepsize \rfloor$ log-density + grad evaluations for \smallCap{hmc} (w/ $\integTime$ fixed).
	}%
\end{tightItemize}
